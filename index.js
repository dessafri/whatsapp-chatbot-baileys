  ...
  global.sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      try {
        ...
        const phoneNumber = getPhoneNumber(processedMsg.from);
        if (phoneNumber) {
          rememberJid(phoneNumber, processedMsg.from);
        }
...
async function checkIsOperatorHandle(msgdata) {
  var phoneNumber = getPhoneNumber(msgdata.from);
    ...
    if (rstMenuCommand.length > 0) {
      ...
      if (operatorCommand[0].value == rstMenuCommand[0].code) {
        ...
        await queryNew(
          ...
          [getPhoneNumber(msgdata.from)]
        );
        
        pusher.trigger("my-channel", "my-event", {
          message: "Anda Punya Permintaan Bantuan Operator! dari nomor whatsapp : " + getPhoneNumber(msgdata.from) + " Mohon Segera Cek",
          phoneNumber: getPhoneNumber(msgdata.from),
        });
...
async function handleRegistrationLink(msgdata, message, timestamp, baseUrl, welcomeMsg) {
  try {
    const phoneNumber = getPhoneNumber(msgdata.from);
...
async function cekUserFirst(msgData) {
  try {
    const phoneNumber = getPhoneNumber(msgData.from);
    rememberJid(phoneNumber, msgData.from);
          ...
          <div class="container">
            ...
            <div class="instructions">
              ...
              <ol>
                ...
                <li>Tap <strong>⋮</strong> (titik tiga) atau <strong>⚙️ Settings</strong></li>
            ...
            <div class="refresh-note">
              ⏱️ Halaman akan refresh otomatis setiap 30 detik
            </div>
          </div>
        </body>
      </html>
